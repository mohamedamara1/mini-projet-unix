...
void on_submit_filename_button_clicked(GtkButton *button, gpointer user_data) {
    // Get the filename from the entry widget
    const char *filename = gtk_entry_get_text(GTK_ENTRY(user_data));

    // Close the filename entry window
 //   gtk_widget_destroy(GTK_WIDGET(gtk_widget_get_parent(GTK_WIDGET(user_data))));
...
void on_show_file_content_button_clicked(GtkButton *button, gpointer user_data) {
    // Create a new pop-up window for entering the file name
    GtkWidget *filename_popup_window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
    gtk_window_set_title(GTK_WINDOW(filename_popup_window), "Enter File Name");
    gtk_container_set_border_width(GTK_CONTAINER(filename_popup_window), 10);
    gtk_widget_set_size_request(filename_popup_window, 300, 100);

    // Create a grid for layout
    GtkWidget *filename_grid = gtk_grid_new();
    gtk_container_add(GTK_CONTAINER(filename_popup_window), filename_grid);

    // Create and add an entry widget for the file name
    GtkWidget *filename_entry = gtk_entry_new();
    gtk_entry_set_placeholder_text(GTK_ENTRY(filename_entry), "Enter File Name");
    gtk_grid_attach(GTK_GRID(filename_grid), filename_entry, 0, 0, 1, 1);

    // Create and add a button to submit the file name
    GtkWidget *submit_button = gtk_button_new_with_label("Submit");
    g_signal_connect(submit_button, "clicked", G_CALLBACK(on_submit_filename_button_clicked), filename_entry);
    gtk_grid_attach(GTK_GRID(filename_grid), submit_button, 0, 1, 1, 1);

    // Show all widgets in the pop-up window
    gtk_widget_show_all(filename_popup_window);
}
